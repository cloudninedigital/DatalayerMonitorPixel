...
___INFO___
{
  ...
  "description": "Implement the shadowpixel from Cloud Nine Digital. Sends a pixel with page/dataLayer context (event, domain, device, browser, section, live-session data) to the Cloud Nine Digital collection endpoint.",
  "containerContexts": [
    "WEB"
  ]
}
...
[
  {
    ...
    "help": "The Cloud Nine Digital collection endpoint this pixel should send data to. This is provided by your Cloud Nine Digital consultant, or found in your DLM setup documentation."
  },
  {
    "type": "TEXT",
    "name": "section",
    "displayName": "Page Type (DLM variable)",
    "simpleValueType": true,
    "help": "Enter the value of the dynamic DLM page-type variable for this page (e.g. 'homepage', 'checkout', 'article'). Typically populated from a dataLayer variable that reflects the page's content type, and is passed through as the 'section' value in the pixel payload."
  },
  {
    "type": "TEXT",
    "name": "domain",
    ...
    "simpleValueType": true,
    "help": "Enter the client ID for this domain. This is provided by your Cloud Nine Digital consultant, or found in your DLM setup documentation. For example, 'www.cloudninedigital.nl' is abbreviated as 'CND'."
  },
  {
    "type": "TEXT",
    "name": "userAgent",
    "displayName": "Enter the userAgent Custom Javascript variable",
    "simpleValueType": true,
    "help": "Create a Custom JavaScript variable that returns navigator.userAgent, and reference it here (e.g. {{CJS - userAgent}})."
  }
]
 ...
 * Changelog:
 *   - 1.0 (2026-07-15): Baseline version. Pixel URL always
 *     concatenates endPoint + domain, no endpoint-based fallback.
 * ═════════════════════════════════════════════
 */
...
if (dataLayer && gtmId) {
  obj = dataLayer.map(function(o) {
    ...
    return o && o['gtm.uniqueEventId'] === gtmId;
  });
}
...
function getLiveSessionId() {
  var values = getCookieValues(LIVE_COOKIE_NAME);
  return values && values.length ? values[0] : null;
}
var url = getUrl();
var liveParam = getQueryParam(url, 'dlm_live_session');
var liveSessionIdParam = getQueryParam(url, 'dlm_live_session_id');
var liveSessionId = getLiveSessionId();
if ((liveParam === 'true' || liveSessionIdParam)) {
  liveSessionId = generateSessionId();
  if (liveSessionIdParam) {
  liveSessionId = liveSessionIdParam;
  }
  setCookie(LIVE_COOKIE_NAME, liveSessionId, {
    'max-age': 60 * 60,
    'path': '/'
  });
}
...
var payload = {
  event: latestEntry['event'] || 'unknown',
  domain: domain,
  url: url,
  device_type: getDeviceType(),
  browser: getBrowser(userAgent),
  section: section,
  datalayer_payload: dataLayerPayload
};
if (isLiveSession) {
payload.session_id = liveSessionId;
payload.session_start_timestamp = liveSessionId;
}
// 🔗 Build query string
var queryString = '';
...
// 📡 Endpoint switch
var endpointPath = isLiveSession ? '/datalayers-img-live/?': '/datalayers-img/?';
var pixelUrl = endPoint + '/' + domain + endpointPath + queryString;
// 🧪 Log live events
if (isLiveSession) {
  logToConsole('DLM LIVE EVENT', {
    endpoint: pixelUrl,
    payload: payload
  });
}
...
Republished as v1.0 on 15/07/2026 for Community Template Gallery submission.
